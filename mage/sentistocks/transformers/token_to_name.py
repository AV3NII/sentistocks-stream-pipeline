...
def transform(data: DataFrame, *args, **kwargs):
    try:
        # Debug print to check initial DataFrame structure
        print(data.show())
...
def test_output(output, *args) -> None:
    ...
    Ensures that the output is not None and checks basic DataFrame properties.
    """
    assert output is not None, 'The output is undefined'
    assert isinstance(output, DataFrame), 'Output is not a DataFrame'
    assert 'Currency' in output.columns, "DataFrame does not contain the 'Currency' column"
